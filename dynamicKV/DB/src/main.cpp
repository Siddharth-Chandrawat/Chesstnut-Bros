...
int main() {
  ...
  // POST /{model} - Create model and add/update a key if data is provided
  CROW_ROUTE(app, "/<string>").methods("POST"_method)(
          [&config, &model_engines, &get_engine](
              ...
              try {
                  ...
                  // The request must contain the KV key
                  if (!json.contains("key") || !json["key"].is_string()) {
                      return crow::response(400, "Missing or invalid key");
                  }

                  // Extract the actual KV key
                  std::string key = json["key"].get<std::string>();

                  // Remove "key" so only the stored value remains
                  json.erase("key");

                  // Store the complete JSON object under the requested key
                  engine->put(key, json.dump());
                  ...
                  return crow::response(500, "Failed to store data");
              }
